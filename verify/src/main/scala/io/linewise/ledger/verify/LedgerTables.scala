...
object LedgerTables {
  ...
  def entriesToLegs(txId: FMLong, es: List[LedgerEntry], n: BigInt): List[LegRow] =
  ...
  // ---- first-order recursive predicates over the ledger tables (no List.contains on FMLong) ----
  def legsOf(legs: List[LegRow], id: FMLong): List[LegRow] = legs.filter((l: LegRow) => l.txId == id)
  def noLegWith(legs: List[LegRow], id: FMLong): Boolean =
    if legs.isEmpty then true else legs.head.txId != id && noLegWith(legs.tail, id)
  def noHeaderWith(hs: List[TxHeaderRow], id: FMLong): Boolean =
    if hs.isEmpty then true else hs.head.id != id && noHeaderWith(hs.tail, id)
  def allSameTx(legs: List[LegRow], id: FMLong): Boolean =
  ...
  sealed abstract class LedgerStore {
    ...
    def get(db: DB, id: FMLong): Option[LedgerTx]
  ...
  case class JdbcLedgerStore() extends LedgerStore {
    ...
    @extern @pure
    def get(db: DB, id: FMLong): Option[LedgerTx] = Db.txById(id)
    @extern @pure
