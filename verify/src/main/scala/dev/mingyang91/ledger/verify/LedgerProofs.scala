...
object LedgerProofs {
  ...
  def entriesAllSame(id: Long, es: List[LedgerEntry], n: BigInt): Unit = {
    es match { case Nil() => (); case Cons(e, t) => entriesAllSame(id, t, n + BigInt(1)) }
  }.ensuring(_ => allSameTx(entriesToLegs(id, es, n), id))

  def entriesNonEmpty(id: Long, es: List[LedgerEntry], n: BigInt): Unit = { () }
    .ensuring(_ => entriesToLegs(id, es, n).isEmpty == es.isEmpty)

  def posCorr(id: Long, es: List[LedgerEntry], n: BigInt): Unit = {
  ...
  def dirCorr(id: Long, es: List[LedgerEntry], n: BigInt, d: EntryDirection): Unit = {
  ...
  def sumCorr(id: Long, es: List[LedgerEntry], n: BigInt, d: EntryDirection): Unit = {
  ...
  @induct def legsOfAllSame(a: List[LegRow], id: Long): Unit = { require(allSameTx(a, id)); () }
    .ensuring(_ => legsOf(a, id) == a)
  @induct def legsOfNoLeg(a: List[LegRow], id: Long): Unit = { require(noLegWith(a, id)); () }
    .ensuring(_ => legsOf(a, id) == (Nil[LegRow](): List[LegRow]))
  @induct def legsOfAppend(a: List[LegRow], b: List[LegRow], id: Long): Unit = { () }
    .ensuring(_ => legsOf(a ++ b, id) == legsOf(a, id) ++ legsOf(b, id))
  def legsOfFreshAppend2(newLegs: List[LegRow], oldLegs: List[LegRow], id: Long): Unit = {
  ...
  @induct def legsOfFreshAppend(newLegs: List[LegRow], oldLegs: List[LegRow], freshId: Long, hhId: Long): Unit = {
  ...
  def admissibleStableUnderFreshLegs(hs: List[TxHeaderRow], oldLegs: List[LegRow], newLegs: List[LegRow], freshId: Long): Unit = {
