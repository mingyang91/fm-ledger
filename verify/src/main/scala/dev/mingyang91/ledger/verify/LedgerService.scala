...
case class LedgerService[W](has: Has[W, DB], store: LedgerStore) {
  ...
  private def txIdFresh(db: DB, id: Long): Boolean = store.get(db, id).isEmpty
  ...
  def credit(
      ...
      amount: Long, freshId: Long, sourceKind: String, sourceId: String): (W, Either[LedgerError, LedgerTx]) =
