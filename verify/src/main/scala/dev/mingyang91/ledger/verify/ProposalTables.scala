...
object ProposalTables {
  ...
  def assembleP(r: ProposalRow, st: ProposalStatus, resultTx: Option[Long]): Proposal =
    Proposal(r.id, r.kind, r.userUid, r.debitAccount, r.creditAccount, r.amount, r.reason, r.proposedBy, st, resultTx, r.targetTxId)

  def latest(ss: List[PStatusRow], id: Long): Option[PStatusRow] =
    ss.find((s: PStatusRow) => s.proposalId == id)

  sealed abstract class ProposalStore {
    def propose(db: DB, p: Proposal): DB
    def decide(db: DB, id: Long, toStatus: ProposalStatus, resultTx: Option[Long]): DB
    def get(db: DB, id: Long): Option[Proposal]
    def all(db: DB): List[Proposal]
  }

  case class InMemProposalStore() extends ProposalStore {
    def propose(db: DB, p: Proposal): DB =
      db.copy(proposals = toPRow(p) :: db.proposals, pstatuses = PStatusRow(p.id, ProposalStatus.PendingReview, None[Long]()) :: db.pstatuses)
    def decide(db: DB, id: Long, toStatus: ProposalStatus, resultTx: Option[Long]): DB =
      ...
      db.proposals.find((r: ProposalRow) => r.id == id) match
        case Some(r) =>
          latest(db.pstatuses, id) match
            ...
            case _       => Some[Proposal](assembleP(r, ProposalStatus.PendingReview, None[Long]()))
    ...
    def all(db: DB): List[Proposal] =
      db.proposals.map((r: ProposalRow) =>
        latest(db.pstatuses, r.id) match
          ...
          case _       => assembleP(r, ProposalStatus.PendingReview, None[Long]()))
  }
  ...
  case class JdbcProposalStore() extends ProposalStore {
    ...
    @extern @pure
    def get(db: DB, id: Long): Option[Proposal] = Db.proposalById(id)
    @extern @pure
