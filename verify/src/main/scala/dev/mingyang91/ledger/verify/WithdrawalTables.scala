...
object WithdrawalTables {
  ...
  def currentStatus(ss: List[WStatusRow], id: Long): Option[WithdrawalStatus] =
    ss.find((s: WStatusRow) => s.withdrawalId == id).map((s: WStatusRow) => s.toStatus)
  def currentStatusOr(ss: List[WStatusRow], id: Long): WithdrawalStatus =
  ...
  sealed abstract class WithdrawalStore {
    ...
    def transition(db: DB, id: Long, toStatus: WithdrawalStatus): DB
    def get(db: DB, id: Long): Option[Withdrawal]
  ...
  case class JdbcWithdrawalStore() extends WithdrawalStore {
    ...
    @extern @pure
    def get(db: DB, id: Long): Option[Withdrawal] = Db.withdrawalById(id)
    @extern @pure
    def findByClientReq(db: DB, userUid: String, clientRequestId: String): Option[Withdrawal] =
