...
object PayoutLifecycleProofs {
  ...
  private def zero: Long = 0L

  def findWithdrawal(rows: List[Withdrawal], id: Long): Option[Withdrawal] =
  ...
  def findIntentByWithdrawal(rows: List[PayoutIntent], withdrawalId: Long): Option[PayoutIntent] =
    rows.find((i: PayoutIntent) => i.withdrawalId == withdrawalId)

  def findIntentById(rows: List[PayoutIntent], id: Long): Option[PayoutIntent] =
    rows.find((i: PayoutIntent) => i.id == id)

  def findDispatch(rows: List[PayoutDispatch], intentId: Long): Option[PayoutDispatch] =
  ...
  def findProviderEventForWithdrawal(rows: List[ProviderEvent], withdrawalId: Long, outcome: ProviderOutcome): Option[ProviderEvent] =
    rows.find((e: ProviderEvent) => e.withdrawalId == withdrawalId && e.outcome == outcome)

  def eventCountForWithdrawal(rows: List[ProviderEvent], withdrawalId: Long): BigInt =
    rows match
      case Nil() => BigInt(0)
      case Cons(h, tail) =>
        (if h.withdrawalId == withdrawalId then BigInt(1) else BigInt(0)) + eventCountForWithdrawal(tail, withdrawalId)

  def settledEventCountForWithdrawal(rows: List[ProviderEvent], withdrawalId: Long): BigInt =
    rows match
      case Nil() => BigInt(0)
      case Cons(h, tail) =>
  ...
  def failedEventCountForWithdrawal(rows: List[ProviderEvent], withdrawalId: Long): BigInt =
    rows match
      case Nil() => BigInt(0)
      case Cons(h, tail) =>
  ...
  def reconciliationCountForIntent(rows: List[PayoutReconciliation], intentId: Long): BigInt =
    rows match
      case Nil() => BigInt(0)
      case Cons(h, tail) =>
        (if h.intentId == intentId then BigInt(1) else BigInt(0)) + reconciliationCountForIntent(tail, intentId)

  def containsIntentByWithdrawal(rows: List[PayoutIntent], withdrawalId: Long): Boolean =
  ...
  def containsIntentId(rows: List[PayoutIntent], id: Long): Boolean =
    rows.exists((i: PayoutIntent) => i.id == id)

  def containsDispatchId(rows: List[PayoutDispatch], intentId: Long): Boolean =
  ...
  def containsEventWithdrawal(rows: List[ProviderEvent], withdrawalId: Long): Boolean =
  ...
  def containsReconciliationIntent(rows: List[PayoutReconciliation], intentId: Long): Boolean =
  ...
  def quoteValid(wd: Withdrawal, req: PayoutSubmit): Boolean =
    req.quotedFee >= zero && req.expectedNet > zero && req.amountMinor > zero && req.idempotencyKey != "" &&
      wd.amount == wrapping { req.expectedNet + req.quotedFee }
  ...
  def validIntentForWithdrawal(intent: PayoutIntent, wd: Withdrawal): Boolean =
    intent.withdrawalId == wd.id && intent.userUid == wd.userUid &&
      ...
      intent.gross == wrapping { intent.expectedNet + intent.quotedFee }
  ...
  def submittedCouplingHolds(w: PayoutWorld, withdrawalId: Long): Boolean =
  ...
  def settledCouplingHolds(w: PayoutWorld, withdrawalId: Long): Boolean =
    findIntentByWithdrawal(w.intents, withdrawalId) match
      case Some(intent) =>
  ...
  def failedCouplingHolds(w: PayoutWorld, withdrawalId: Long): Boolean =
    findIntentByWithdrawal(w.intents, withdrawalId) match
      case Some(intent) =>
      ...
      case _ => false
  def exactSubmitDelta(before: PayoutWorld, after: PayoutWorld, wd: Withdrawal, req: PayoutSubmit, freshIntentId: Long): Boolean = {
  ...
  def exactSettledDelta(before: PayoutWorld, after: PayoutWorld, wd: Withdrawal, intent: PayoutIntent, eventId: String, observedFee: Long, transferRef: Option[String], ids: SettlementIds, accounts: SettlementAccounts): Boolean = {
  ...
  def exactFailedDelta(before: PayoutWorld, after: PayoutWorld, wd: Withdrawal, provider: String, eventId: String, observedFee: Long, transferRef: Option[String], freshTxId: Long, clearingAccount: String, userAccount: String): Boolean = {
  ...
  def submit(w: PayoutWorld, withdrawalId: Long, req: PayoutSubmit, freshIntentId: Long): (PayoutWorld, Either[PayoutError, PayoutIntent]) =
  ...
  def claim(w: PayoutWorld, intentId: Long): PayoutWorld =
  ...
  def dispatchSuccess(w: PayoutWorld, intentId: Long, transferRef: String): PayoutWorld =
    findDispatch(w.dispatches, intentId) match
  ...
  def dispatchFailure(w: PayoutWorld, intentId: Long, retryable: Boolean, maxAttempts: BigInt): PayoutWorld =
    findDispatch(w.dispatches, intentId) match
      case Some(d) if d.status == DispatchStatus.InFlight =>
        val nextAttempts = d.attempts + BigInt(1)
        val nextStatus = if !retryable || nextAttempts >= maxAttempts then DispatchStatus.Failed else DispatchStatus.Pending
  ...
  def reconciliationResult(expected: Long, observed: Long): ReconciliationResult =
  ...
  def submitMissingWithdrawalLeavesWorldUnchanged(w: PayoutWorld, withdrawalId: Long, req: PayoutSubmit, freshIntentId: Long): Boolean = {
  ...
  def submitBadQuoteLeavesWorldUnchanged(w: PayoutWorld, wd: Withdrawal, req: PayoutSubmit, freshIntentId: Long): Boolean = {
  ...
  def submitWithProviderTransferRefRejected(w: PayoutWorld, wd: Withdrawal, req: PayoutSubmit, freshIntentId: Long): Boolean = {
    require(!req.providerTransferRef.isEmpty)
    val w0 = w.copy(withdrawals = putWithdrawal(w.withdrawals, wd))
    submit(w0, wd.id, req, freshIntentId) == (w0, Left[PayoutError, PayoutIntent](PayoutError.BadTransferRef))
  }.holds

  def submitSuccessCreatesIntentAndDispatch(w: PayoutWorld, wd: Withdrawal, req: PayoutSubmit, freshIntentId: Long): Boolean = {
  ...
  def submitExistingSameIntentIsIdempotent(w: PayoutWorld, withdrawalId: Long, req: PayoutSubmit, freshIntentId: Long): Boolean = {
    require(req.providerTransferRef.isEmpty)
  ...
  def submitExistingDifferentIntentConflicts(w: PayoutWorld, withdrawalId: Long, req: PayoutSubmit, freshIntentId: Long): Boolean = {
    require(req.providerTransferRef.isEmpty)
    require(findWithdrawal(w.withdrawals, withdrawalId) match
      case Some(wd) => findIntentByWithdrawal(w.intents, withdrawalId) match
        case Some(intent) => !intentMatches(wd, req, intent)
  ...
  def submitDifferentPayloadConflictLeavesWorldUnchanged(w: PayoutWorld, withdrawalId: Long, req: PayoutSubmit, freshIntentId: Long): Boolean = {
    require(req.providerTransferRef.isEmpty)
    require(findWithdrawal(w.withdrawals, withdrawalId) match
      case Some(wd) => findIntentByWithdrawal(w.intents, withdrawalId) match
        case Some(intent) => !intentMatches(wd, req, intent)
  ...
  def submitWrongStatusLeavesWorldUnchanged(w: PayoutWorld, withdrawalId: Long, req: PayoutSubmit, freshIntentId: Long): Boolean = {
    require(findWithdrawal(w.withdrawals, withdrawalId) match
  ...
  def submitSuccessHasExactDelta(w: PayoutWorld, wd: Withdrawal, req: PayoutSubmit, freshIntentId: Long): Boolean = {
    require(wd.status == WithdrawalStatus.PendingReview)
    require(quoteValid(wd, req))
    require(req.providerTransferRef.isEmpty)
    require(req.expectedStatus == WithdrawalStatus.PendingReview)
  ...
  def submitPreservesLedgerRows(w: PayoutWorld, wd: Withdrawal, req: PayoutSubmit, freshIntentId: Long): Boolean = {
    require(wd.status == WithdrawalStatus.PendingReview)
    require(quoteValid(wd, req))
    require(req.providerTransferRef.isEmpty)
    require(req.expectedStatus == WithdrawalStatus.PendingReview)
  ...
  def negativeObservedFeeRejected(w: PayoutWorld, provider: String, eventId: String, withdrawalId: Long, observedFee: Long, ids: SettlementIds, accounts: SettlementAccounts): Boolean = {
  ...
  def settledMissingWithdrawalLeavesWorldUnchanged(w: PayoutWorld, provider: String, eventId: String, withdrawalId: Long, observedFee: Long, ids: SettlementIds, accounts: SettlementAccounts): Boolean = {
    require(observedFee >= zero)
    require(findWithdrawal(w.withdrawals, withdrawalId).isEmpty)
  ...
  def settledMissingIntentLeavesWorldUnchanged(w: PayoutWorld, wd: Withdrawal, provider: String, eventId: String, observedFee: Long, ids: SettlementIds, accounts: SettlementAccounts): Boolean = {
    require(observedFee >= zero)
    require(findIntentByWithdrawal(w.intents, wd.id).isEmpty)
    require(findProviderEvent(w.events, provider, eventId).isEmpty)
    val w0 = w.copy(withdrawals = putWithdrawal(w.withdrawals, wd))
  ...
  def settledWrongStatusLeavesWorldUnchanged(w: PayoutWorld, wd: Withdrawal, intent: PayoutIntent, eventId: String, observedFee: Long, ids: SettlementIds, accounts: SettlementAccounts): Boolean = {
    require(observedFee >= zero)
  ...
  def settledWithIntentWritesReconciliation(w: PayoutWorld, wd: Withdrawal, intent: PayoutIntent, eventId: String, observedFee: Long, ids: SettlementIds, accounts: SettlementAccounts): Boolean = {
    require(wd.status == WithdrawalStatus.Submitted)
    require(validIntentForWithdrawal(intent, wd))
    require(observedFee >= zero)
  ...
  def settledWithIntentHasExactDelta(w: PayoutWorld, wd: Withdrawal, intent: PayoutIntent, eventId: String, observedFee: Long, ids: SettlementIds, accounts: SettlementAccounts): Boolean = {
    require(wd.status == WithdrawalStatus.Submitted)
    require(validIntentForWithdrawal(intent, wd))
    require(observedFee >= zero)
    require(findProviderEvent(w.events, intent.provider, eventId).isEmpty)
  ...
  def settledWithIntentCouplesStatus(w: PayoutWorld, wd: Withdrawal, intent: PayoutIntent, eventId: String, observedFee: Long, ids: SettlementIds, accounts: SettlementAccounts): Boolean = {
    require(wd.status == WithdrawalStatus.Submitted)
    require(validIntentForWithdrawal(intent, wd))
    require(observedFee >= zero)
    val w0 = w.copy(withdrawals = putWithdrawal(w.withdrawals, wd), intents = Cons(intent, Nil[PayoutIntent]()), events = Nil[ProviderEvent](), reconciliations = Nil[PayoutReconciliation]())
  ...
  def failedMissingWithdrawalLeavesWorldUnchanged(w: PayoutWorld, provider: String, eventId: String, withdrawalId: Long, observedFee: Long, freshTxId: Long, clearingAccount: String, userAccount: String): Boolean = {
    require(observedFee >= zero)
    require(findWithdrawal(w.withdrawals, withdrawalId).isEmpty)
    require(findProviderEvent(w.events, provider, eventId).isEmpty)
  ...
  def failedWrongStatusLeavesWorldUnchanged(w: PayoutWorld, wd: Withdrawal, provider: String, eventId: String, observedFee: Long, freshTxId: Long, clearingAccount: String, userAccount: String): Boolean = {
    require(observedFee >= zero)
    require(wd.status != WithdrawalStatus.Submitted)
    require(findProviderEvent(w.events, provider, eventId).isEmpty)
    val w0 = w.copy(withdrawals = putWithdrawal(w.withdrawals, wd))
  ...
  def failedWebhookWritesReturnAndNoReconciliation(w: PayoutWorld, wd: Withdrawal, provider: String, eventId: String, observedFee: Long, freshTxId: Long, clearingAccount: String, userAccount: String): Boolean = {
    require(observedFee >= zero)
    require(wd.status == WithdrawalStatus.Submitted)
    require(findProviderEvent(w.events, provider, eventId).isEmpty)
  ...
  def failedWebhookHasExactDelta(w: PayoutWorld, wd: Withdrawal, provider: String, eventId: String, observedFee: Long, freshTxId: Long, clearingAccount: String, userAccount: String): Boolean = {
    require(observedFee >= zero)
    require(wd.status == WithdrawalStatus.Submitted)
    require(findProviderEvent(w.events, provider, eventId).isEmpty)
    val w0 = w.copy(withdrawals = putWithdrawal(w.withdrawals, wd))
  ...
  def failedWebhookCouplesStatus(w: PayoutWorld, wd: Withdrawal, provider: String, eventId: String, observedFee: Long, freshTxId: Long, clearingAccount: String, userAccount: String): Boolean = {
    require(observedFee >= zero)
    require(wd.status == WithdrawalStatus.Submitted)
    val intent = PayoutIntent(1L, wd.id, wd.userUid, provider, userAccount, wd.amount, zero, wd.amount, None[String]())
    val w0 = w.copy(withdrawals = putWithdrawal(w.withdrawals, wd), intents = Cons(intent, Nil[PayoutIntent]()), events = Nil[ProviderEvent](), reconciliations = Nil[PayoutReconciliation]())
    processFailed(w0, provider, eventId, wd.id, observedFee, None[String](), freshTxId, clearingAccount, userAccount) match
   ...
   * ------------------------------------------------------------------------ */
  def reclaim(w: PayoutWorld, intentId: Long, stale: Boolean): PayoutWorld =
    findDispatch(w.dispatches, intentId) match
  ...
  def dispatchPermanentFailureRefunds(
      w: PayoutWorld, intentId: Long, retryable: Boolean, maxAttempts: BigInt,
      freshTxId: Long, clearingAccount: String, userAccount: String): PayoutWorld =
    findDispatch(w.dispatches, intentId) match
      case Some(d) if d.status == DispatchStatus.InFlight =>
        val nextAttempts = d.attempts + BigInt(1)
  ...
  def permanentFailureFailsWithdrawalAndRefunds(w: PayoutWorld, d: PayoutDispatch, wd: Withdrawal, maxAttempts: BigInt, freshTxId: Long, clearingAccount: String, userAccount: String): Boolean = {
    require(d.status == DispatchStatus.InFlight)
    require(wd.status == WithdrawalStatus.Submitted)
  ...
  def permanentFailureNeverLeavesWithdrawalSubmitted(w: PayoutWorld, d: PayoutDispatch, wd: Withdrawal, maxAttempts: BigInt, freshTxId: Long, clearingAccount: String, userAccount: String): Boolean = {
    require(d.status == DispatchStatus.InFlight)
    require(wd.status == WithdrawalStatus.Submitted)
    require(d.withdrawalId == wd.id)
    val w0 = w.copy(dispatches = putDispatch(w.dispatches, d), withdrawals = putWithdrawal(w.withdrawals, wd))
  ...
  def retryableFailureDoesNotRefund(w: PayoutWorld, d: PayoutDispatch, wd: Withdrawal, maxAttempts: BigInt, freshTxId: Long, clearingAccount: String, userAccount: String): Boolean = {
    require(d.status == DispatchStatus.InFlight)
    require(wd.status == WithdrawalStatus.Submitted)
    require(d.withdrawalId == wd.id)
    require(d.attempts + BigInt(1) < maxAttempts)
