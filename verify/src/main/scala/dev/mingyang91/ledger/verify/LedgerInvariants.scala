 ...
 * never List.contains, so Stainless sees the induction shape directly.
 * ========================================================================== */
object LedgerInvariants {

  // ---------- LEDGER (header + leg) ----------
  def hasHeader(hs: List[TxHeaderRow], id: Long): Boolean = hs match
    case Nil()      => false
  ...
  def dirSum(legs: List[LegRow], d: EntryDirection): Long = legs match
    case Nil()      => 0L
    case Cons(l, t) => wrapping { (if l.dir == d then l.amount else 0L) + dirSum(t, d) }
  ...
  def allLegsPositive(legs: List[LegRow]): Boolean = legs match
    case Nil()      => true
    case Cons(l, t) => l.amount > 0L && allLegsPositive(t)
  ...
  def hasWithdrawal(ws: List[WithdrawalRow], id: Long): Boolean = ws match
    case Nil()      => false
    case Cons(w, t) => w.id == id || hasWithdrawal(t, id)
  def noWithdrawalWith(ws: List[WithdrawalRow], id: Long): Boolean = ws match
    case Nil()      => true
  ...
  def validWithdrawalRows(rows: List[Withdrawal]): Boolean = rows.forall((wd: Withdrawal) => wd.amount > 0L)

  // ---------- PROPOSAL (row + status event-stream) ----------
  def hasProposal(ps: List[ProposalRow], id: Long): Boolean = ps match
    case Nil()      => false
    case Cons(p, t) => p.id == id || hasProposal(t, id)
  def noProposalWith(ps: List[ProposalRow], id: Long): Boolean = ps match
    case Nil()      => true
  ...
  def hasIntent(is: List[IntentRow], id: Long): Boolean = is match
    case Nil()      => false
    case Cons(i, t) => i.id == id || hasIntent(t, id)
  def noIntentWith(is: List[IntentRow], id: Long): Boolean = is match
    case Nil()      => true
