...
object PayoutProofs {
  ...
  case class SettlementIds(netTxId: Long, feeRecoveryTxId: Long, providerFeeTxId: Long)
  case class SettlementAccounts(clearing: String, providerBalance: String, feeRecovery: String, providerFeeExpense: String)
  case class RecipientPaysFeeQuote(gross: Long, expectedRecipientNet: Long, quotedProviderFee: Long)
  ...
  def quoteMatches(q: RecipientPaysFeeQuote): Boolean =
    q.gross == wrapping { q.expectedRecipientNet + q.quotedProviderFee }

  def nonNegative(amount: Long): Boolean = amount >= zero

  def feeRecoveryTx(id: Long, userUid: String, amount: Long, accounts: SettlementAccounts): Option[LedgerTx] =
    if amount > zero then
  ...
  def providerFeeTx(id: Long, userUid: String, amount: Long, accounts: SettlementAccounts): Option[LedgerTx] =
    if amount > zero then
  ...
  def optionalAmount(tx: Option[LedgerTx]): Long =
    tx match
      case Some(t) => t.amount
      case _       => 0L

  def clearingReleased(s: PayoutSettlement): Long =
    wrapping { s.netTx.amount + optionalAmount(s.feeRecoveryTx) }

  def feeRecovered(s: PayoutSettlement): Long =
    optionalAmount(s.feeRecoveryTx)

  def providerFeeExpensed(s: PayoutSettlement): Long =
  ...
  def reconciliationResult(expectedFee: Long, observedFee: Long): String =
    if observedFee == expectedFee then "matched" else "fee_variance"

  def recipientPaysFeeSettlementAdmissible(
      ids: SettlementIds, userUid: String, q: RecipientPaysFeeQuote, observedProviderFee: Long, accounts: SettlementAccounts): Boolean = {
    ...
    LedgerValidation.admissible(s.netTx) && optionalAdmissible(s.feeRecoveryTx) && optionalAdmissible(s.providerFeeTx) &&
      clearingReleased(s) == q.gross
  }.holds

  def zeroQuotedFeeSkipsRecovery(ids: SettlementIds, userUid: String, gross: Long, observedProviderFee: Long, accounts: SettlementAccounts): Boolean = {
  ...
  def feeVarianceIsAuditable(expectedFee: Long, observedFee: Long): Boolean = {
