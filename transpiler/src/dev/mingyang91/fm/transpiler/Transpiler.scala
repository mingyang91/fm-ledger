package dev.mingyang91.fm.transpiler
 ...
 * node, a `Term.Apply(Term.Name("wrapping"), _)`, a `.ensuring` `Term.Select`),
 ...
 *   imports  drop `import stainless.*`, `import *Proofs.*`; REDIRECT
 *            `dev.mingyang91.verify.ox` -> `ox` and
 *            `dev.mingyang91.verify.effect.SafeArith` -> `dev.mingyang91.fm.SafeArith`
 ...
 *   R9       erase `wrapping { e }` to `{ e }`; Scala Long already wraps at runtime
 *   R4       Some[T](x)/None[T]()/Nil[T]() -> Some(x)/None/Nil
 * ========================================================================== */
object Transpiler {
  ...
  def header(srcRel: String): String =
    s"""// =============================================================================
       ...
       |// substitutions applied (stainless imports dropped, `wrapping` blocks erased,
       |// proof-only contracts/annotations stripped, and
       |// Some[T](x)/None[T]()/Nil[T]() collapsed to the Scala standard forms).
  ...
  def transpile(input: String, srcRel: String, targetPkg: String = "dev.mingyang91.fm.generated"): String = {
  ...
  def rewrite(input: String, targetPkg: String = "dev.mingyang91.fm.generated"): String = {
    var s = input
    s = importsRule(s)
  ...
  //   dev.mingyang91.verify.ox               -> redirect ref to `ox`
  //   dev.mingyang91.verify.effect.SafeArith -> redirect ref to dev.mingyang91.fm
  ...
  private def importsRule(s: String): String = astRule(s) {
    case imp: Import =>
      ...
      if path == "dev.mingyang91.verify.ox" then
        redirect("ox")
      else if path.startsWith("dev.mingyang91.verify.effect.SafeArith") then
        // `import dev.mingyang91.verify.effect.SafeArith._` (SafeArith is the ref tail)
        redirect(path.replace("dev.mingyang91.verify.effect.SafeArith", "dev.mingyang91.fm.SafeArith"))
      else if path == "dev.mingyang91.verify.effect" && importees.exists(_.contains("SafeArith")) then
        // `import dev.mingyang91.verify.effect.SafeArith` (SafeArith is the importee)
        redirect("dev.mingyang91.fm")
      else if root == "stainless" then dropLine
      else if path == "dev.mingyang91.verify.effect" then dropLine
  ...
  // --- R9: erase Stainless wrapping marker -----------------------------------
  private def wrappingRule(s: String): String = astRule(s) {
    case t @ Term.Apply(Term.Name("wrapping"), List(body)) =>
      List(Edit(t.pos.start, t.pos.end, text(s, body)))
  }
